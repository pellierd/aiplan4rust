...
pub fn test_pnf_positive_normalization_all_files(domain_dir: &Path) -> bool {
    ...
    for problem_path in &problems_to_process {
        ...
        // --- 1. Execute PNF Pass (Standalone production call) ---
        if let Err(e) = to_pnf(&mut pb) {
            println!(
                "  - PNF CRASH: Positive transformation failed on {:?}: {:?}",
                problem_path, e
            );
            success = false;
            continue;
        ...
        // --- 2. Call External PNF Verification Oracle ---
        if !check_pnf_constraints(&pb, problem_path) {
            success = false;
        }
    }
...
/// Verifies that a LiftedProblem strictly complies with the Positive PNF constraints.
/// Returns `true` if the problem is valid, `false` if any unabsorbed 'Not' operators are found.
pub fn check_pnf_constraints(pb: &LiftedProblem, problem_path: &Path) -> bool {
    let store = pb.store();
    ...
    for derived in pb.derived_predicate_defs() {
        pure_conditions.push(derived.body());
    }

    for method in pb.method_defs() {
        pure_conditions.push(method.precondition());
    }
...
pub fn test_pnf_positive_normalization(domain_path: &str) {
    let path = Path::new(domain_path);
    assert!(
        test_pnf_positive_normalization_all_files(path),
        "PNF positive compilation pipeline left explicit negations for directory: {}",
        domain_path
    );
}
