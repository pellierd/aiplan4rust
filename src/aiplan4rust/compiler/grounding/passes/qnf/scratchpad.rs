...
/// traversals, a flat dual lookup table for sub-expression sharing, and flat buffers
/// for dynamic expression interning.
///
/// # Arguments
/// * `cache` - A high-speed flat lookup table indexed by `ExprId::as_usize()`, providing O(1) memoization.
...
impl QnfScratchpad {
    ...
    pub fn new() -> Self {
        Self {
            cache: Vec::new(), // Will be sized and initialized on-demand via `.clear(store_len)`
    ...
    /// Clears all internal collections and resizes the lookup cache to synchronize
    /// with the current `ExprStore` state, ensuring zero heap allocations on hot paths.
    pub fn clear(&mut self, store_len: usize) {
        // SIMD-driven vectorized fill operation: resets the cache in O(1) space allocations
        self.cache.resize(store_len, ExprId::default());
        self.cache.fill(ExprId::default());
