    ...
    pub fn identify_segment(&self, sk_id: AtomSkeletonId) -> (&'static str, String) {
        ...
        if let Some(type_idx) = self.type_to_skeleton.iter().position(|&s| s == sk_id) {
            let name = self
                .problem
                .type_symbols()
                .get_ident(crate::aiplan4rust::support::lang::id::TypeId::from(
                    type_idx,
                ))
        ...
        if raw_id < self.fluence_threshold {
            let name = self
                .problem
                .predicate_symbols()
                .get_ident(crate::aiplan4rust::support::lang::id::PredicateSymbolId::from(raw_id))
                .and_then(|&sym| interner.resolve_symbol(sym))
        ...
        if raw_id >= self.action_base_id && raw_id < self.action_threshold {
            ...
            let name = self
                .problem
                .action_symbols()
                .get_ident(crate::aiplan4rust::support::lang::id::ActionSymbolId::from(
                    act_idx,
                ))
                .and_then(|&sym| interner.resolve_symbol(sym))
