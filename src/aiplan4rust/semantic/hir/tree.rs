...
///   for a domain.
pub type LiftedProblem = Tree;
pub type LiftedDomain = Tree;
...
pub struct Tree {
    ...
    syntax_tree: LinkedHashMap<usize, HirNode>,
...
impl Tree {
    pub fn get_parent(&self, id: usize) -> Option<&HirNode> {
    ...
    pub fn source(&self) -> SymbolSource {
        if let Some(first_node) = self.syntax_tree.values().next() {
            match first_node.kind() {
                AstKind::Domain => SymbolSource::Domain,
                AstKind::Problem => SymbolSource::Problem,
    ...
    pub fn get_entry(&self, id: usize) -> Option<&HirNode> {
        self.syntax_tree.get(&id)
    }

    pub fn add_entry(&mut self, id: usize, entry: HirNode) {
    ...
    pub fn iter(&self) -> impl Iterator<Item = (&usize, &HirNode)> {
        self.syntax_tree.iter()
    }

    pub fn iter_mut(&mut self) -> impl Iterator<Item = (&usize, &mut HirNode)> {
    ...
    pub fn values(&self) -> impl Iterator<Item = &HirNode> {
    ...
    pub fn from(syntax_tree: &Ast) -> Result<Self, ParserInternalError> {
    ...
    /// # Arguments
    ///
    /// * `ast` - A reference to the annotated syntax tree containing all parsed nodes.
    ///
    /// # Returns
    ///
    ...
    fn extract_requirements(
        ...
        for node in syntax_tree.values() {
            if let AstKind::Requirement(req) = node.kind() {
