...
///
/// This structure acts as a "diff" between the semantic analysis (which identifies
/// redundancies) and the mutation phase (which updates the SymbolTable and
/// prepares the AST finalization).
struct TypeSimplification {
    /// The unique identifier of the symbol in the Symbol Table.
    /// Used to quickly locate the `SymbolEntry` to modify without string lookups.
    symbol_id: SymbolId,

    /// The unique identifier of the declaration node in the AST.
    /// Points to the specific occurrence of the declaration, which is essential
    /// if a symbol is declared multiple times across the domain or problem.
    node_id: NodeId,

    /// The new semantic definition of the type after simplification.
    /// Contains the optimized type union (e.g., `satellite` instead of `(either satellite object)`).
    new_type: Type<SymbolId>,

    /// The original indices of the union members that were kept.
    ///
    /// For example, given `(either A B C)` where `B` is removed, the value will be `[0, 2]`.
    /// This attribute is critical for the finalization pass: it tells the compiler
    /// which child syntax nodes to physically remove from the AST.
