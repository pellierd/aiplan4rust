...
///
/// # Parameters
/// - `ast`: A mutable reference to the `AnnotatedSyntaxTree` to process.
...
/// - `ast`: The annotated syntax tree from parsing.
...
fn merge_symbol_type_declarations(
    ...
    // Vector to store data needed for diagnostics creation later on, without borrowing ast
    let mut diagnostics_data = Vec::new();
