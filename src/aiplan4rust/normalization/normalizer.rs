...
//! - A fixed sequence of expr passes (e.g., typed list flattening, disjunction simplification).
...
/// Performs AST expr by applying canonical transformation passes.
///
...
impl Normalizer {
    ...
    /// Normalizes the result of parsing by applying standard expr passes.
    ///
    /// This method consumes the input [`ParserResult`], extracts the AST if present,
    /// applies expr passes on it, and returns a [`NormalizerResult`] with the
