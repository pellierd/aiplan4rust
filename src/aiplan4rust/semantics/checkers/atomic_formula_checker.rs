...
/// This struct contains static methods that operate on the tree's symbols and declarations.
pub struct AtomicFormulaChecker;

impl AtomicFormulaChecker {
    ...
    /// if AtomicFormulaChecker::check(&tree, &type_checker, &mut errors).is_ok() {
    ///     // Handle no errors
    /// } else {
    ///     // Handle errors
        ...
        for symbol in symbol_table.values() {
            ...
            for declaration in symbol.declarations() {
                ...
                for usage in symbol.usages() {
                    if !Self::match_declaration_with_usage(
                        declaration,
                        usage,
                        symbol_table,
                        ast_table,
