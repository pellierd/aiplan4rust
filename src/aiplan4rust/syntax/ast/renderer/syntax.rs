...
pub fn render(
    ...
    match node.kind() {
        ...
        AstKind::TaskDef => {
            ...
            if let Some(&params_id) = children.get(idx) {
                if let Some(params_node) = arena.get_node(params_id) {
                    writeln!(f)?;
                    write!(f, "{}:parameters (", AstNode::make_indent(indent + 1))?;
        ...
        AstKind::MethodDef => {
            ...
            let indent_param = AstNode::make_indent(indent + 1);
            ...
            if let Some(&body_id) = children.get(2) {
                ...
                if let Some(body_node) = arena.get_node(body_id) {
                    ...
                    let indent_body = AstNode::make_indent(indent + 1);
        ...
        AstKind::OrderedSubtaskDef | AstKind::PartiallyOrderedSubtaskDef => {
            ...
            if let Some(and_id) = children.next() {
                if let Some(and_node) = arena.get_node(*and_id) {
                    ...
                    and_node.kind().fmt_syntax(f, interner)?; // prints "and"
                                ...
                                match task_node.kind() {
                                    ...
                                    other => {
                                        ...
                                        writeln!(
                                            f,
                                            "{}<unexpected-{}>",
                                            AstNode::make_indent(indent + 2),
                                            other
                                        )?;
                                    }
                                }
                            } else {
                                writeln!(f, "{}<invalid>", AstNode::make_indent(indent + 1))?;
                            }
                        }
        ...
        AstKind::TaskOrderingConstraint => {
            let indent_str = AstNode::make_indent(indent); // Compute indent string once

            // Write opening parenthesis with indentation
            write!(f, "{}(", indent_str)?;

            // Print the content (e.g., "<")
            node.content().fmt_syntax(f, interner)?;
        ...
        AstKind::ActionDefBody => {
            let children = node.children();
            let indent_child = AstNode::make_indent(indent);
        ...
        | AstKind::TaskID => {
            write!(f, "{}", indent_str)?;
            node.content().fmt_syntax_with_indent(f, interner, indent)
        }
