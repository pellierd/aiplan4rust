...
use crate::aiplan4rust::syntax::ast_old::serialize::SerializableNode;
...
pub struct AstNodeOld {
    id: usize,
    kind: AstKindOld,
    children: Vec<Box<AstNodeOld>>,
    span: Span,
}
...
impl AstNodeOld {
    ...
    /// ```
    pub fn new(kind: AstKindOld, children: Vec<Box<AstNodeOld>>, start: usize, end: usize) -> AstNodeOld {
        AstNodeOld {
            id: usize::MAX,
            kind,
            children,
    ...
    /// # Returns
    /// A new `Node` instance.
    pub fn new_with_span(kind: AstKindOld, children: Vec<Box<AstNodeOld>>, span: Span) -> AstNodeOld {
        AstNodeOld {
            id: usize::MAX,
            kind,
            children,
    ...
    pub fn kind_mut(&mut self) -> &mut AstKindOld {
        &mut self.kind
    }
    ...
    /// ```
    pub fn set_kind(&mut self, new_kind: AstKindOld) {
        self.kind = new_kind;
    }

    /// Returns an immutable reference to the child nodes.
    pub fn children(&self) -> &Vec<Box<AstNodeOld>> {
        &self.children
    }

    /// Returns a mutable reference to the child nodes.
    pub fn children_mut(&mut self) -> &mut Vec<Box<AstNodeOld>> {
        &mut self.children
    }

    /// Replaces the child nodes with a new vector.
    pub fn set_children(&mut self, new_children: Vec<Box<AstNodeOld>>) {
    ...
    pub fn assign_unique_ids(&mut self, start_id: usize) {
        fn helper(node: &mut AstNodeOld, counter: &mut usize) {
            node.id = *counter;
            *counter += 1;
    ...
    pub fn check_ids_unique(&self) -> bool {
        fn helper(node: &AstNodeOld, seen: &mut HashSet<usize>) -> bool {
...
impl<'de> Deserialize<'de> for AstNodeOld {
...
impl fmt::Display for AstNodeOld {
...
impl SyntaxDisplay for AstNodeOld {
    ...
    fn to_syntax_string_with_depth(&self, depth: usize) -> String {
        ...
        match &self.kind() {
            ...
            AstKindOld::TypedList => {
                ...
                for (i, child) in self.children().iter().enumerate() {
                    if i > 0
                        && !(child.kind == AstKindOld::TypedList && child.children().is_empty())
                    {
                        write!(str, " ").unwrap();
                    }
                    if matches!(child.kind(), AstKindOld::Type) {
                        write!(str, "- ").unwrap();
                    }
                    write!(str, "{}", child.to_syntax_string()).unwrap();
                }
            ...
            AstKindOld::ActionDefBody => {
                for child in self.children() {
                    write!(str, "\n{}", child.to_syntax_string_with_depth(depth)).unwrap();
                }
            }
            AstKindOld::PreconditionDef => {
                write!(
                    str,
                    "{}{}\n{}",
                    offset,
