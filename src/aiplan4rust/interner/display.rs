...
pub trait SelfInternerDisplay {
    ...
    where
        Self: Sized,
    {
        self.try_to_string().expect("Failed to render SelfInternerDisplay")
    }
}

/// Wrapper struct used to implement [`std::fmt::Display`] for any type implementing [`SelfInternerDisplay`].
///
