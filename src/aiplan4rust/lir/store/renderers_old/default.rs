...
use crate::aiplan4rust::lir::store::expr_old::{Expr, ExprNode};
use crate::aiplan4rust::lir::store::problem_old::{DomainDef, LiftedProblem, ProblemDef};
use crate::aiplan4rust::lir::store::renderers_old::common::{
    render_labeled_expr, render_labeled_typed_list, writeln_centered,
};
use crate::aiplan4rust::lir::{
    ActionDef, DerivedPredicateDef, InitialTaskNetwork, MethodDef, TaskNetwork,
};
use std::fmt;
...
pub fn render_problem(f: &mut fmt::Formatter<'_>, problem: &LiftedProblem) -> std::fmt::Result {
    ...
    if !problem.predicate_defs().is_empty() {
        ...
        for (i, p) in problem.predicate_defs().iter().enumerate() {
            ...
            // Rendu final aligné
            writeln!(f, "  {:<5} : {:<15} {}", id_str, name, params_str)?;
        }
        writeln!(f, "{:-<80}\n", "")?;
    }
...
pub fn render_action(f: &mut fmt::Formatter<'_>, action: &ActionDef) -> std::fmt::Result {
    ...
    // On adapte le label selon le typing d'action pour rester proche de la sémantique PDDL
...
pub fn render_node_expr_content(f: &mut fmt::Formatter<'_>, content: &Content) -> fmt::Result {
    match content {
