...
impl TypeRegistry {
    /// Creates a new `TypeRegistry` by indexing existing type definitions from a borrowed `TypedList`.
    ///
    ...
    ///
    /// # Arguments
    /// * `type_defs` - A reference to the [`TypedList`] holding the problem's initial type hierarchy.
    ///
    /// # Returns
    /// An initialized registry with the counter set at the end of the existing ID space.
    ///
    /// # Implementation Detail
    /// By taking a reference `&TypedList`, this function avoids any heap allocations or copies
    /// of the underlying symbol list, maintaining a strict **$\mathcal{O}(1)$ dynamic allocation profile**
    /// during the indexing phase.
    pub fn new(type_defs: &TypedList<TypeId, TypeId>) -> Self {
        ...
        for def in type_defs.iter() {
            ...
            // We only cache composite types (e.g., 'Either' unions) as atomic types are self-resolving.
            if members.len() > 1 {
                // .to_vec() ici est nécessaire pour créer la clé du cache (Vec<TypeId>)
                let mut sig: Vec<TypeId> = members.to_vec();
                sig.sort_unstable();
                sig.dedup();

                // On insère le symbole (qui est déjà un TypeId) comme valeur cible
