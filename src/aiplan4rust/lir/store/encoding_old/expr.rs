...
//! ## Overview
//!
//! The encoding_old process takes a [`SyntaxSubtree<AstNode>`] and produces a flat,
...
//! 2.  **Symbol Resolution**: During encoding_old, every identifier in the AST is
...
use crate::aiplan4rust::lir::store::encoding_old::{typed_list, EncodingRegistry};
use crate::aiplan4rust::lir::store::expr_old::{Expr, ExprContent, ExprError, ExprKind, ExprNode};
...
///
/// This function serves as the primary orchestrator for the expression encoding_old process.
...
/// * `Err(LirError)` - If any part of the encoding_old or symbol resolution fails.
///
/// # Process Flow
///
...
/// * `Err(LirError)` - If the node fails to encoding_old or if a parent-child link cannot be established.
///
/// # Process
///
...
/// * `evaluator` - The mutable encoding_old evaluator used for symbol table lookups and variable registration.
///
/// # Returns
///
...
///
/// This function acts as a semantic filter during the encoding_old process. It ensures that
