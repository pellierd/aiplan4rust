...
use crate::aiplan4rust::lir::store::expr_old::ops::error::ExprOpError;
use crate::aiplan4rust::lir::store::expr_old::ops::simplification::when;
...
/// ```
/*pub fn simplify(node_id: NodeId, expr_old: &mut Expr) -> Result<(), ExprOpError> {
    // Step 1: Flatten nested AND/OR nodes of the same kind
    flatten_and_or_node(node_id, expr_old)?;

    // Step 2: Sort children in canonical order
    canonicalize_and_or_node(node_id, expr_old)?;

    // Step 3: Deduplicate structurally
    deduplicate_and_or_node(node_id, expr_old)?;

    // Step 4: Merge WHEN logic
    if expr_old.node_kind(node_id) == Some(ExprKind::And) {
        merge_when(node_id, expr_old)?;
    }

    // Step 5: Simplify tautologies and contradictions
    if simplify_tautologies_and_contradictions(node_id, expr_old)? {
        return Ok(());
    }

    // Step 6: Reduce nodes with a single child
    if reduce_single_and_or_node(node_id, expr_old)? {
        return Ok(());
    }

    // Step 7: Simplify empty nodes
    simplify_empty_and_or_node(node_id, expr_old)?;
...
/*fn simplify_empty_and_or_node(node_id: NodeId, expr_old: &mut Expr) -> Result<bool, ExprOpError> {
    // 1. On récupère les infos nécessaires sans bloquer l'arène
    let (node_kind, children) = {
        let node = expr_old.try_node(node_id)?;
    ...
    for child_id in children {
        let child = expr_old.try_node(child_id)?;
        ...
        if child.children().is_empty() {
                ...
                || (node_kind == ExprKind::Or && child_kind == ExprKind::And)
            {
                ...
                expr_old.move_to(child_id, node_id)?;
                return Ok(true);
            }
    ...
    if modified {
        expr_old.try_node_mut(node_id)?.set_children(new_children);
    }
...
pub fn collect_and_merge_when(
    ...
    // Pour pouvoir utiliser 'expr_old' mutablement plus tard (hash/deep_eq),
