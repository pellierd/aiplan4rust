...
pub fn fuse_nested_quantifiers(node_id: NodeId, expr: &mut Expr) -> Result<(), ExprOpError> {
    ...
    let (outer_kind, inner_id) = {
        ...
        debug_assert!(
            ...
            "Outer quantifier must have exactly one child"
        );
    ...
    debug_assert!(
        inner_node.children().len() == 1,
        "Inner quantifier must have exactly one child"
    );
...
/// ```
fn simplify_quantifier_trivial_body(node_id: NodeId, expr: &mut Expr) -> Result<bool, ExprOpError> {
...
mod tests {
    ...
    use crate::aiplan4rust::lir::store::expr_old::ops::simplification::quantifier;
    use crate::aiplan4rust::lir::store::expr_old::ops::ExprOpError;
