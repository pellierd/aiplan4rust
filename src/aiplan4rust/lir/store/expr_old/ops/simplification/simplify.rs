use crate::aiplan4rust::lir::store::expr_old::ops::simplification::{
    and_or, arithmetic, assign, comparison, not, quantifier, when,
};
use crate::aiplan4rust::lir::store::expr_old::ops::{ExprOpError, StaticEvaluator, StaticValue};
...
/*pub fn simplify_subexpr_with(
    ...
    for id in ids {
        simplify_node(id, expr_old, evaluator)?;
    }
...
/*pub fn simplify_subexpr_with(
    ...
    for id in ids {
        ...
        let node_ref = match expr_old.get_node(id) {
            Some(n) => n,
            None => {
    ...
    while let Some(step) = stack.pop() {
        match step {
            Step::Discover(id) => {
                ...
                // On récupère les IDs via un emprunt court pour ne pas bloquer expr_old
            ...
            Step::Process(id) => {
                ...
                if is_simplifiable(kind_before, has_eval) {
                    ...
                    // Mutation de l'expr_old (le Borrow Checker est ok car la stack ne contient que des IDs/Enums)
