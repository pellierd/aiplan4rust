...
//!
//! It processes nodes that pair identifiers with their either_type definitions, ensuring
...
///
/// This function extracts a symbol name and resolves its associated either_type(s). It is
/// commonly used for parsing action parameters, constants in a domain, or objects
/// in a problem definition.
///
/// # Arguments
///
/// * `subtree` - The syntax subtree representing the symbol and its optional either_type.
/// * `evaluator` - The encoding context used to resolve either_type identifiers.
///
/// # Returns
///
/// * `Ok(TypedSymbol<TypeID>)` - The encoded symbol with its resolved numeric TypeIDs.
/// * `Err(LirError)` - If the identifier is missing or either_type resolution fails.
///
/// # Errors
///
/// This function will return an error if:
/// * The first child (the symbol name) cannot be resolved to a valid identifier.
/// * The second child (the either_type definition), if present, fails the encoding process
///   (e.g., refers to an unregistered either_type).
pub fn encode_typed_type(
    ...
    // The second child contains the either_type definitions (e.g., 'type1' or an 'either' block)
    let ty = if children.len() > 1 {
        let ty_node_id = children[1];
        let ty_node = ast.try_node(ty_node_id)?;
        // Delegate to the either_type encoding module to resolve the TypeID(s)
        ty::encode(&SyntaxSubtree::new(ty_node, ty_node_id, ast), registry)?
    } else {
        // Fallback to an empty Type (representing 'object' or untyped) if no either_type is provided
        Type::new()
    };
...
pub fn encode_typed_object(
    ...
    // 3. Encodage du either_type (à droite du tiret)
