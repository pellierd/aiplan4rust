...
/*pub fn encode(
    ...
    // 1. Extraction du label (p1, p2, etc.)
    // PDDL: (preference <label> <gd>) -> Le label est le premier enfant.
    let label_node_id = node.try_child(0)?;
    let label_node = tree.try_node(label_node_id)?;
    let label_symbol = label_node.try_ident()?;

    // 2. IDENTITÉ : On réserve l'ID du symbole dans le problème et le registre
    // On utilise la méthode "à la task label" (register_preference_symbol)
    let preference_id = registry.register_preference_symbol(label_symbol);
    ir.add_preference_symbol(label_symbol);

    // 3. MAPPING : On lie le NodeId de l'AST à l'ID du LIR pour la phase de logique
    registry.register_preference(label_node_id, preference_id);

    // 4. ENCODAGE DE LA LOGIQUE : On transforme la condition (GD) en Expr
    // Le corps de la préférence est le deuxième enfant.
    let body_node_id = node.try_child(1)?;
    let body_node = tree.try_node(body_node_id)?;
    let body_subtree = SyntaxSubtree::new(body_node, body_node_id, tree);

    let condition_expr = expr::encode(&body_subtree, registry)?;

    // 5. STOCKAGE : On enregistre la définition dans le Problem
    // L'ID sert d'index direct dans le Vec<Expr> de ir.preference_defs
    ir.add_preference_def(preference_id, condition_expr);
