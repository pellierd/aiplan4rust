...
fn flatten_arithmetic_expression(node_id: NodeId, expr: &mut Expr) -> Result<(), ExprOpError> {
    let node = expr.try_node(node_id)?;
...
/// * `Err(ExprError)` if an arithmetic evaluation fails (e.g., division by zero).
fn reduce(node_id: NodeId, expr: &mut Expr) -> Result<(), ExprOpError> {
    let node = expr.try_node(node_id)?;
...
fn apply_partial_reduction(
    ...
    if !is_neutral(final_constant, op) {
        ...
        let const_node = ExprNode::new(ExprKind::Number, Content::Number(final_constant), None);
...
fn evaluate_arithmetic_expression(
    ...
    let result = match op {
        ...
        ArithmeticOp::Div => rest.iter().try_fold(*first, |acc, x| {
            if x.0 == 0.0 {
                Err(ExprOpError::arithmetic_evaluation_error(
                    op,
                    values.to_vec(),
                ))
            } else {
